...
namespace Ara3D.Speckle.Data
{
    public static class SpeckleUtils
    {
        ...
        public static IEnumerable<Base> PullModelsFromProject(this Client client, string projectId, ILogger logger = null)
            => client.GetModels(projectId).Select(m => client.PullModelFromId(projectId, m.id, logger));
        ...
        public static Base PullModel(this Client client, string projectId, string modelName, ILogger logger)
        {
            logger?.Log($"Pulling model {modelName} from project {projectId}");
            var model = client.GetModelOrDefault(projectId, modelName);
            if (model == null)
                throw new Exception($"Model {modelName} not found in project {projectId}");
            logger?.Log($"Model Id = {model.id}");
            return client.PullModelFromId(projectId, modelName, logger);
        }
